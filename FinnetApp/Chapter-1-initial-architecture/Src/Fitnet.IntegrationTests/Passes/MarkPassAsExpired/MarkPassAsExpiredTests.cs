    ...
    IClassFixture<DatabaseContainer>, IAsyncLifetime
{
    ...
    {
        using var getAllPassesResponse = await _applicationHttpClient.GetAsync("api/passes");
        var response = await getAllPassesResponse.Content.ReadFromJsonAsync<GetAllPassesJsonResponse>();
        var createdPass = response!.Passes.FirstOrDefault(pass => pass.CustomerId == customerId);

        return createdPass;
    }

    private static string BuildUrl(Guid id) => $"api/passes/{id}";

    private sealed record GetAllPassesJsonResponse(IReadOnlyCollection<PassDto> Passes);
